...
def check_bias_by_body_length(labeled_df, predicted_df):
    ...
    for task in tasks:
        ...
        def binary_score(row):
            _, _, bert_f1 = compute_bert_score(row[y_pred_col], row[y_true_col], model_type="roberta-large")
            ...
            return int(1 if (bert_f1 >= thresholds["rougeL"] and ner_coverage >= thresholds_ner["ner"] ) else 0)
